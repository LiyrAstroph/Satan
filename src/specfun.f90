...
SUBROUTINE gser(gamser,a,x,gln)
      ...
      PARAMETER (ITMAX=1000000,EPS=3.d-7)
!U    USES gammln
      INTEGER n
...
11    continue
      write(*,*)'a too large, ITMAX too small in gser', a
      stop
...
SUBROUTINE gcf(gammcf,a,x,gln)
      ...
      PARAMETER (ITMAX=10000000,EPS=3.d-7,FPMIN=1.d-30)
!U    USES gammln
      INTEGER i
...
11    continue
      write(*,*)'a too large, ITMAX too small in gcf', a
      stop
...
function inverfc(p)
   ...
   t=sqrt(-2.0*dlog(pp/2.0))
   x=-0.70711*((2.30753+t*0.27061)/(1.+t*(0.99229+t*0.04481)) - t)
   do i=0, 2, 1
     errx=erfc(x) - pp
     x =x+ errx/(1.12837916709551257*dexp(-x*x)-x*errx)
   enddo
   if(p.lt.1.0)then
     inverfc=x
   else
